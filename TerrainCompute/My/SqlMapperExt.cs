...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly Dictionary<RuntimeTypeHandle, string> GetQueries = new Dictionary<RuntimeTypeHandle, string>();
        private static readonly Dictionary<RuntimeTypeHandle, string> TypeTableName = new Dictionary<RuntimeTypeHandle, string>();
        ...
        private static PropertyInfo GetSingleKey<T>(string method)
        {
            ...
            if (keyCount > 1)
                throw new DataException(string.Format("{method}<T> only supports an entity with a single [Key] or [ExplicitKey] property. [Key] Count: {keys.Count}, [ExplicitKey] Count: {explicitKeys.Count}",method,keys.Count,explicitKeys.Count));
            if (keyCount == 0)
        ...
        /*public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
            {
                ...
                sql = string.Format("select * from {name} where {key.Name} = @id",name,key.Name);
        ...
        */
        ...
        private static string GetTableName(Type type)
        {
            ...
            else
            {
                ...
                var tt =
                    info.GetCustomAttribute<TableAttribute>(false);
                if(tt==null){
                    tt=(info.GetCustomAttributes(false).FirstOrDefault(attr => attr.GetType().Name == "TableAttribute") as dynamic);
                    
                }else{
                    tt=tt.Name;
                }
                var tableAttrName=tt;
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            else
            {
                ...
                var cmd = string.Format("insert into {name} ({sbColumnList}) values ({sbParameterList})",name,sbColumnList,sbParameterList);
        ...
        {
            IProxy proxy=null;
            if(entityToUpdate is IProxy){
                proxy=(IProxy)entityToUpdate;
            }
        ...
        public static bool DeleteAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            var name = GetTableName(type);
            var statement = string.Format("delete from {name}",name);
        ...
        private static ISqlAdapter GetFormatter(IDbConnection connection)
        {
            ...
            if(GetDatabaseType==null){
                ...
                name=GetDatabaseType.Invoke(connection).ToLower();
                if(name==null){
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    {
        var cmd = string.Format("insert into {tableName} ({columnList}) values ({parameterList});select SCOPE_IDENTITY() id",tableName,columnList,parameterList);
...
public partial class SqlCeServerAdapter : ISqlAdapter
{
    ...
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        var cmd = string.Format("insert into {tableName} ({columnList}) values ({parameterList})",tableName,columnList,parameterList);
...
public partial class MySqlAdapter : ISqlAdapter
{
    ...
    {
        var cmd = string.Format("insert into {tableName} ({columnList}) values ({parameterList})",tableName,columnList,parameterList);
        connection.Execute(cmd, entityToInsert, transaction, (CommandType)commandTimeout);
...
public partial class SQLiteAdapter : ISqlAdapter
{
    ...
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        var cmd = string.Format("INSERT INTO {tableName} ({columnList}) VALUES ({parameterList}); SELECT last_insert_rowid() id",tableName,columnList,parameterList);
        var multi = connection.QueryMultiple(cmd, entityToInsert, transaction, commandTimeout);
...
public partial class FbAdapter : ISqlAdapter
{
    ...
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        var r = connection.Query(string.Format("SELECT FIRST 1 {keyName} ID FROM {tableName} ORDER BY {keyName} DESC",keyName,tableName,keyName), transaction: transaction, commandTimeout: commandTimeout);
