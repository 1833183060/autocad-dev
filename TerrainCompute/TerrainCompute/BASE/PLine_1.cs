...
{	//ng:原始版本
	public class PLine_1 : List<Edge>
	{
		static PLine_1()
		{
			PLine_1.edgeVertexList = new EdgeVertexList();
			PLine_1.string_0 = "Y";
			PLine_1.string_1 = "Y";
			PLine_1.int_0 = 8;
			PLine_1.double_0 = 0.0;
			PLine_1.string_2 = "N";
		}
		...
		public static List<PLine_1> ConstructFromUnorderedSegments(List<Edge> unorderedEdges, int relevantDecimals, ref int numberOfPLines, ref int numberOfSegments, ref int zeroLength, bool writeDatabase, bool useProgressMeter, bool eliminateZeroLengthSegments)
		{
			...
			List<PLine_1> list = new List<PLine_1>();
			PLine_1.edgeVertexList = new EdgeVertexList();
			if (useProgressMeter)
			{
			...
			try
			{
				...
				for (int i = 0; i < unorderedEdges.Count; i++)
				{
					...
					PLine_1.edgeVertexList.Add(edge);
				}
				for (int j = 0; j < unorderedEdges.Count; j++)
				{
					...
					{
						PLine_1 pLine = new PLine_1();
						pLine.caddata_0 = unorderedEdges[j].CADData;
						List<Edge> list2 = PLine_1.smethod_0(unorderedEdges[j]);
						pLine.AddRange(list2);
						pLine.Add(unorderedEdges[j]);
						unorderedEdges[j].Status = Edge.EdgeStatus.NotDefined;
						List<Edge> list3 = PLine_1.smethod_1(unorderedEdges[j]);
		...
		public void LinesToPolylines()
		{
			...
			try
			{
				...
				PLine_1.int_0 = CommandLineQuerries.SpecifyInteger("Specify number of relevant decimal digits", PLine_1.int_0, 0, 12, false, true);
				PLine_1.string_1 = CommandLineQuerries.KeywordYesNo("Eliminate zero length segments", PLine_1.string_1, false, false);
				PLine_1.string_0 = CommandLineQuerries.KeywordYesNo("Delete original lines", PLine_1.string_0, false, false);
		...
		private void method_0(ObjectId[] objectId_0)
		{
			...
			try
			{
				...
				if (PLine_1.string_1.Trim().ToUpper() == "N")
				{
					eliminateZeroLengthSegments = false;
				}
				PLine_1.ConstructFromUnorderedSegments(unorderedEdges, PLine_1.int_0, ref num2, ref num, ref num3, true, true, eliminateZeroLengthSegments);
				if (PLine_1.string_0 == "Y")
				{
				...
				using (Transaction transaction = workingDatabase.TransactionManager.StartTransaction())
				{
					...
					for (int i = 0; i < objectId_0.Length; i++)
					{
						try
						{
							...
							if (PLine_1.string_2 == "N" & !@object.IsErased)
							{
								@object.Erase();
							}
		...
		public void SimplifyPolylinesCommand()
		{
			...
			try
			{
				...
				PLine_1.double_0 = CommandLineQuerries.SpecifyDouble("Specify epsilon range", PLine_1.double_0, false, false, false, true);
				PLine_1.string_2 = CommandLineQuerries.KeywordYesNo("Keep original polylines", PLine_1.string_2, false, false);
				int num = 0;
				int num2 = 0;
				int num3 = 0;
				this.method_1(array, PLine_1.double_0, ref num, ref num2, ref num3);
		...
		private static List<Edge> _smethod_0(Edge edge_0)
		{
			...
			while (edge.Status == Edge.EdgeStatus.NotDefined)
			{
				...
				List<Edge> list2 = PLine_1.edgeVertexList[edge.StartPoint];
				if (list2.Count != 0)
				{
					if (list2.Count != 1)
					{
        ...
        private static List<Edge> smethod_0(Edge edge_0)
        {//去掉原代码的goto等
            Editor arg_0F_0 = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor;
            List<Edge> list = new List<Edge>();
            Edge edge = edge_0;
		...
		private static List<Edge> smethod_1(Edge edge_0)
		{
            Editor arg_0F_0 = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor;
			List<Edge> list = new List<Edge>();
			Edge edge = edge_0;
			...
			{
				edge.Status = Edge.EdgeStatus.Defined;
				List<Edge> list2 = PLine_1.edgeVertexList[edge.EndPoint];
				if (list2.Count != 0)
				{
					if (list2.Count != 1)
                    {
