...
{
	public class 单点引注X1_PL : DrawJig
	{
		[DebuggerNonUserCode]
		public 单点引注X1_PL()
		{
		...
		protected override SamplerStatus Sampler(JigPrompts prompts)
		{
			...
			if (value != this.point3d_1)
			{
				...
				{
					p..ctor(value.X + this.double_1 * 1.1, value.Y, 0.0);
				...
				{
					p..ctor(value.X + this.double_1 * 1.1, this.point3d_2.Y, 0.0);
				...
				if (this.point3d_2.X < value.X)
				{
					pointXY2 = CAD.GetPointXY(p, 300.0 * this.double_0, 0.0);
				}
				else
				{
					pointXY2 = CAD.GetPointXY(p, -300.0 * this.double_0, 0.0);
