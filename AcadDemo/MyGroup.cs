...
namespace AcadUtil
{
    ...
    {
        public static ObjectId erasingGroupId = ObjectId.Null;

        public static void AddIds2Group(Group g, ObjectIdCollection ids)
        {
            g.InsertAt(0, ids);
        }

        public static void AddIds2Group(string groupName, ObjectIdCollection ids,Transaction trans)
        {
            Group g = getGroup(groupName, trans,OpenMode.ForWrite);
            g.InsertAt(0, ids);
        }
        ...
        public static Group getGroup(string groupName, Transaction trans, OpenMode openMode= OpenMode.ForRead)
        {
            //定义数据库
            Database db = HostApplicationServices.WorkingDatabase;
            ...
            {
                //获取组对象
                Group group = trans.GetObject((ObjectId)ide.Value, openMode) as Group;
        ...
        public static Group getGroup(StringOption stropt, Transaction trans)
        {            
            //定义数据库
            Database db = HostApplicationServices.WorkingDatabase;
            ...
            {
                //获取组对象
                Group group = trans.GetObject((ObjectId)ide.Value, OpenMode.ForRead) as Group;

                if (stropt.Match(group.Name))
                {
                    return group;                   
                }
            }
